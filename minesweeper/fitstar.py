...
class FitMS(object):
     ...
     def run(self,*args,**kwargs):
          ...
          # check to see if user wants to run with isochrone prior, if so 
          # change the parameters it wants to cycle through
          if inputdict.get('isochrone_prior',True):
               ...
               for pp in isopars:
                    ...
                    if self.phot_bool:
                         self.fitpars_bool['Dist'] = True
               self.fitargs['ageweight'] = inputdict.get('ageweight',True)
               self.fitargs['mistdif'] = inputdict.get('mistdif',True)

          # determine if any priors are fixed
     ...
     def _rundysampler(self,samplerdict):
          ...
          for it, results in enumerate(dy_sampler.sample_initial(
               ...
               (worst, ustar, vstar, loglstar, logvol, logwt, logz, logzvar,
                    h, nc, worst_it, propidx, propiter, eff, delta_logz,blob) = results             
